...
import os 
import json
# refer to https://github.com/Wakinguup/Underwater_detection/blob/c3d971046cb7d8f6ec8213e9d9fc29c5f28f8412/code/mmdet/datasets/pipelines/transforms.py
@PIPELINES.register_module()
class Mixup(object):
    """Mixup images & bbox
    Args:
    ...
    def __init__(self, prob=0.5, lambd=0.5,
    ...
    def get_img2(self, img1):
        ...
        for annt in self.all_labels['annotations']:  # clw note: can call coco api to get the corresponding ann
            if annt['image_id'] == img2_id:
                labels2.append(np.int64(annt['category_id']-1))
    ...
    def __call__(self, results):
        if random.uniform(0, 1) < self.prob:
            ...
            boxes1 = results['gt_bboxes']
            
            img2, labels2, boxes2, _ = self.get_img2(img1)
            # print(labels2)
            # if labels2 != []:
            #     break
...
#                               show=False, out_file='/home/featurize/work/underwater-object-detection/mixup/' + filename)
#             imshow_det_bboxes(img1, boxes1, labels1, class_names=underwater_classes(), 
#                               show=False, out_file='/home/featurize/work/underwater-object-detection/mixup/' + str(1)+filename)
#             imshow_det_bboxes(img2, boxes2, labels2, class_names=underwater_classes(), 
#                               show=False, out_file='/home/featurize/work/underwater-object-detection/mixup/' + str(2)+filename)
...
@PIPELINES.register_module()
class Mosaic(object):
    def __init__(self, p=0.3, json_path=None, img_path=None):
        self.p = p
        self.json_path = json_path
        self.img_path = img_path
    ...
    def __call__(self, results):
        ...
        if random.random() < self.p:
            H_img4, W_img4, c = results['img_shape']
            xc, yc = W_img4 // 2, H_img4 // 2

            selected_names = rd.sample(np.arange(len(self.all_labels['images']), 3))
            all_names = [filename] + selected_names
            imgs = [results['img']]
            for name in selected_names:
                imgs.append(
                    mmcv.imread(os.path.join(self.train_imgs_folder, name)))

            for i, img in enumerate(imgs):
                h, w = img.shape[0], img.shape[1]
                current_boxes = np.array(
                    self.name_anno_map[all_names[i]]['ann']['bboxes'].copy(),
                    dtype=np.float32)
                current_labels = np.array(
                    self.name_anno_map[all_names[i]]['ann']['labels'].copy(),
                    dtype=np.int64)

                if i == 0:
                    img4 = np.full((H_img4, W_img4, c), 114, dtype=np.uint8)
                    x1a, y1a, x2a, y2a = max(xc - w, 0), max(
                        yc - h,
                        0), xc, yc  # xmin, ymin, xmax, ymax (large image)
                    x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (
                        y2a -
                        y1a), w, h  # xmin, ymin, xmax, ymax (small image)
                    current_boxes = results['gt_bboxes']
                    current_labels = results['gt_labels']
                elif i == 1:
                    x1a, y1a, x2a, y2a = xc, max(yc - h,
                                                 0), min(xc + w, W_img4), yc
                    x1b, y1b, x2b, y2b = 0, h - (y2a - y1a), min(w,
                                                                 x2a - x1a), h
                elif i == 2:
                    x1a, y1a, x2a, y2a = max(xc - w,
                                             0), yc, xc, min(H_img4, yc + h)
                    x1b, y1b, x2b, y2b = w - (x2a - x1a), 0, max(xc, w), min(
                        y2a - y1a, h)
                elif i == 3:  # bottom right
                    x1a, y1a, x2a, y2a = xc, yc, min(xc + w, W_img4), min(
                        H_img4, yc + h)
                    x1b, y1b, x2b, y2b = 0, 0, min(w, x2a - x1a), min(
                        y2a - y1a, h)

                img4[y1a:y2a, x1a:x2a] = img[y1b:y2b, x1b:x2b]
                retrive_bboxes, retrive_labels = self.filter_truth(
                    current_boxes, current_labels, x1b, y1b, x2b - x1b,
                    y2b - y1b, x1a, y1a)

                bboxes.append(retrive_bboxes)
                labels.append(retrive_labels)
    ...
    def filter_truth(bboxes, labels, dx, dy, sx, sy, xd, yd):
        ...
        bboxes[:, 0::2] = np.clip(bboxes[:, 0::2], 0, sx - 1)
        bboxes[:, 1::2] = np.clip(bboxes[:, 1::2], 0, sy - 1)
...
# infer to: https://github.com/zhengye1995/Tianchi-2021-Guangdong-Tile-Detection/blob/main/mmdet/datasets/pipelines/transforms.py
@PIPELINES.register_module()
...
#         img_out = results['img'].copy()
#         imshow_det_bboxes(img_out, results['gt_bboxes'], results['gt_labels'], class_names=underwater_classes(), 
#                           show=False, out_file='/home/featurize/work/underwater-object-detection/autoaug/' + filename)
            
        return results    

# refer to:https://github.com/zhengye1995/Tianchi-2021-Guangdong-Tile-Detection/blob/main/mmdet/datasets/pipelines/transforms.py
@PIPELINES.register_module()
class SoftGridMask(object):
    ...
    def __call__(self, results):
        ...
        results['img'] = results['img'].astype('uint8')
        ## 可视化确认结果无误
        filename = results['img_info']['file_name']
        import cv2
        from mmdet.core.visualization import imshow_det_bboxes
        ...
        imshow_det_bboxes(img_out, results['gt_bboxes'], results['gt_labels'], class_names=underwater_classes(), 
                          show=False, 
                          out_file='/home/featurize/work/underwater-object-detection/gridmask/' + filename)
        return results
...
#         img_out = results['img'].copy()
#         imshow_det_bboxes(img_out, results['gt_bboxes'], results['gt_labels'], class_names=underwater_classes(), 
#                           show=False, out_file='/home/featurize/work/underwater-object-detection/resize/' + filename)
...
#         img_out = results['img'].copy()
#         imshow_det_bboxes(img_out, results['gt_bboxes'], results['gt_labels'], class_names=underwater_classes(), 
#                           show=False, out_file='/home/featurize/work/underwater-object-detection/flip/' + filename)
...
#         img_out = results['img'].copy()
#         imshow_det_bboxes(img_out, results['gt_bboxes'], results['gt_labels'], class_names=underwater_classes(), 
#                           show=False, out_file='/home/featurize/work/underwater-object-detection/image/' + filename)
        # for box, label in zip(results['gt_bboxes']， result['gt_labels']):
    ...
    def __call__(self, results):
        ...
        while True:
            ...
            for i in range(50):
                ...
                img_out = results['img'].copy()
                imshow_det_bboxes(img_out, results['gt_bboxes'], results['gt_labels'], class_names=underwater_classes(), 
                                  show=False, out_file='/home/featurize/work/underwater-object-detection/mcrop/' + filename)
                return results
